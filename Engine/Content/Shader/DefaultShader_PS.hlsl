...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    const float3 ambient = saturate(LightColor * ambientStrength);
	
   
    const float diffuseStrength = 0.5f;
    const float3 n = normalize(input.WorldNormal);
    const float3 l = normalize(LightPosition - input.WorldPosition);
    const float nDotL = dot(n, l);
    const float3 diffuse = saturate(max(nDotL, 0.0f) * LightColor * diffuseStrength);
    ...
    const float3 specular = saturate(pow(max(hDotN, 0.0f), specularPower) * LightColor);
    return float4(albedo * saturate(ambient + diffuse + specular), 1.0f);
}